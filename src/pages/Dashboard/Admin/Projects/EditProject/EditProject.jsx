...
const EditProject = ({ setShowEditProject, selectedProjectData }) => {
	...
	return (
		<div>
			...
			<div className="flex justify-between items-center">
				<h1 className="text-[28px] font-bold">Edit Project</h1>
				<button
			...
			<form onSubmit={handleSubmit(handleUpdateProject)}>
				...
				<div className="mb-5">
					...
					<input
						type="text"
						defaultValue={selectedProjectData.title}
					...
					/>
					{errors.title && <p className="text-red-600 text-left mt-1">{errors.title?.message}</p>}
				</div>

				<div className="mb-5">
					<label className="text-base font-medium leading-5">Project Link</label>
					<input
						type="text"
						defaultValue={selectedProjectData.projectLink}
						...
						className={`w-full  border border-[#FFD333] focus:outline-0 focus:ring-0 focus:ring-transparent focus:border-[1.5px] focus:border-[#FFD333]  rounded-[3px] h-[60px] mt-4  ${
							errors.projectLink && 'focus:border-red-600'
						}`}
					/>
					{errors.projectLink && (
						<p className="text-red-600 text-left mt-1">{errors.projectLink?.message}</p>
					)}
				</div>
